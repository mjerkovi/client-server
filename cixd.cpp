...
void reply_rm(accepted_socket& client_sock, cix_header& header) {
   ...
   //unlink unsucessful most likely due to file not existing in dir
   if(rc < 0) {
      log << "unlink failed: " << strerror(errno) << endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      log << "sending header: " << header << endl;
      send_packet(client_sock, &header, sizeof header);
...
void reply_put (accepted_socket& client_sock, cix_header& header) {
   ...
   ofstream out_file(header.filename, std::ofstream::binary);
   if (!out_file){
      log << "ofstream constructor failed: " << strerror(errno) <<endl;
      header.command = cix_command::NAK;
      header.nbytes = errno;
      log << "sending header: " << header << endl;
      send_packet(client_sock, &header, sizeof header);
...
void reply_get (accepted_socket& client_sock, cix_header& header) {
   ...
   if(!in_file){
      log << "unable to find file in directory: " <<strerror(errno);
      header.command = cix_command::NAK;
      header.nbytes = errno;
      log << "sending header: " << header << endl;
      send_packet(client_sock, &header, sizeof header);
